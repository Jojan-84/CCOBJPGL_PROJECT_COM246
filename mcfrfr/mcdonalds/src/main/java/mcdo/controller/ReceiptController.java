...
public class ReceiptController {
    ...
    @FXML private void initialize() {
        ...
        // Set order type with better formatting
        String orderType = App.getOrderType();
        if (orderType != null && !orderType.isEmpty()) {
            // Create a styled container for order type
            VBox orderTypeBox = new VBox(4);
            orderTypeBox.setStyle("-fx-padding: 8 0; -fx-background-color: #FFF3E0; -fx-background-radius: 4;");
            
            Label orderTypeTitle = new Label("ORDER TYPE");
            orderTypeTitle.setStyle("-fx-font-size: 12px; -fx-text-fill: #666666;");
            
            Label orderTypeValue = new Label(orderType);
            orderTypeValue.setStyle("-fx-font-size: 15px; -fx-font-weight: bold; -fx-text-fill: #000000;");
            
            orderTypeBox.getChildren().addAll(orderTypeTitle, orderTypeValue);
            orderTypeLabel = orderTypeValue; // Keep reference for later use
        } else {
            orderTypeLabel.setText("Dine In");
        }
        
        // Set user information
        String currentUser = App.getCurrentUser();
        if (currentUser != null && !currentUser.isEmpty()) {
            // Add user information to the receipt
            VBox userInfo = new VBox(8);
            userInfo.setStyle("-fx-padding: 8 0;");
            Label userLabel = new Label("Ordered by: " + currentUser);
            userLabel.setStyle("-fx-font-size: 13px; -fx-text-fill: #666666;");
            userInfo.getChildren().add(userLabel);
            
            // Insert user info after the order type
            int orderTypeIndex = itemsContainer.getChildren().indexOf(orderTypeLabel);
            if (orderTypeIndex >= 0) {
                itemsContainer.getChildren().add(orderTypeIndex + 1, userInfo);
            }
        }
        ...
        // Don't clear the cart here anymore - we'll do it after saving
    }
